...
class Products extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['title'] = $this->config->item('site_name').' - Products Management';
    ...
    // POST data 
    $postData = $this->input->post("dd_maincategory");
    
    // load model 
    $this->load->model('Category_model');
    
    // get data 
    $data = $this->Category_model->getCityDepartment($postData);
    ...
    public function add()
    {
        ...
        if($this->input->post())
        {
            ...
            $sCapacity = $this->input->post("tb_Capacity");
            $sProductType = $this->input->post("tb_ProductType");
            $sCode = $this->input->post("tb_Code");
            $sPrice = $this->input->post("tb_Price");
            $sFeatures = $this->input->post("tb_Features");
            $sColors = $this->input->post("tb_Colors");
            $sIdeal = $this->input->post("tb_Ideal");
            ...
            $smaincategory = $this->input->post("dd_maincategory");
            $sCoverImage = ''; $sProductStatus = '';
            ...
            $arrProduct = array(
                ...
                "product_Capacity"   =>  $sCapacity,
                "product_type"   =>  $sProductType,
                "product_Code"   =>  $sCode,
                "product_Price"   =>  $sPrice,
                "product_color"   =>  $sColors,
                "product_isFeatured2"   =>  $sFeatures,
                "product_Idealfor"   =>  $sIdeal,
            ...
            $iInsertedProductId_draft = $this->Products_model->create_product($arrProduct,'_rds_products');
            ...
            if(count($sCategory) > 0)
            {
                for($r=0;$r<count($sCategory);$r++)
                {
                    ...
                    $iLastCatPrdId = $this->Products_model->create_product_data($aCatProducts,'rds_product_categories');
                    $iLastCatPrdId = $this->Products_model->create_product_data($aCatProducts1,'_rds_product_categories');

                }
            }
            if(trim($this->input->post("photo_json")) != '')
            {
                $aPhotoJson = json_decode($this->input->post("photo_json"));
                for($p=0;$p<count($aPhotoJson);$p++)
                {
                    if($sProductStatus == 'active') {
                        $aPhotos = array(
                            "product_id"    =>  $iInsertedProductId,
                            "photo_name" => $aPhotoJson[$p],
                        ...
                        $this->Products_model->create_product($aPhotos1,'_rds_product_photos');
                    }
                    if($sProductStatus == 'draft') {
                        $aPhotos = array(
                            "product_id"    =>  $iInsertedProductId_draft,
                            "photo_name" => $aPhotoJson[$p],
                            "photo_createddate" => date('Y-m-d H:i:s'),
                        ...
                        $this->Products_model->create_product_data($aPhotos,'_rds_product_photos');
                    }
            ...
            if(count($aSpecificationTitle) > 0)
            {
                for($s=0;$s<count($aSpecificationTitle);$s++)
                {
                    if(trim($aSpecificationTitle[$s]) != '')
                    {
                        ...
                        $iPrdSpecId = $this->Products_model->create_product_data($aSpecification,'rds_product_features');
                        ...
                        if(trim($sSpecJson[$s]) != '')
                        {
                            ...
                            for($sp=0;$sp<count($aSpPhoto);$sp++)
                            {
                                ...
                                $this->Products_model->create_product_data($aspPhotos,'rds_product_feature_photo');
            ...
            if(count($aOptionTitle) > 0)
            {
                for($o=0;$o<count($aOptionTitle);$o++)
                {
                    if(trim($aOptionTitle[$o]) != '')
                    {
                        ...
                        $iPrdOptId = $this->Products_model->create_product_data($aOption,'rds_product_features');
...
public function fetchcity()
        {
            if ($this->input->post('state_id'))
            {
             ...
             echo $this->Products_model->fetchstateidwisedata($state_id);
        ...
        if($this->input->post())
        {
            ...
            if(count($sCategory) > 0)
            {
                ...
                for($r=0;$r<count($sCategory);$r++)
                {
                    ...
                    $this->Products_model->create_product_data($aCatProducts,'rds_product_categories');
                    $this->Products_model->create_product_data($aCatProducts,'_rds_product_categories');
                }
            }
            ...
            $this->session->set_flashdata('product', array('message' => 'Prouct updated successfully.','icon' => 'icon fa fa-check','class' => 'alert alert-success alert-dismissible'));
            redirect('admin/products');
        }
        $this->load->view('admin/templates/header', $data);
        $this->load->view('admin/templates/sidebar');
    ...
    public function uploadPrdSpPhoto()
    {
        ...
        $UploadPath = '';
        if($feature_type == 'sp') { $UploadPath = "./uploads/products/specifications/";} else { $UploadPath = "./uploads/products/options/";}
       $arFiles = array();
        if(!is_array($_FILES["file_sp_photo"]["name"])) //single file
        {
            ...
            {
                echo $this->upload->display_errors();die("dd");
            }
            else {
