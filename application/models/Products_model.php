...
class Products_model extends CI_Model
{
	 ...
	 public function fetchstateidwisedata($state_id)
    {
       ...
       $this->db->select('*');
            ...
            $this->db->where('ManuCtegory_id', $state_id);
            $query  = $this->db->get();
    ...
    public function get_products($id = FALSE)
    {
        if ($id === FALSE)
        {
            $this->db->select('product_id, product_modelno,product_type, product_name,product_Capacity,product_isFeatured2,product_description,product_color, ,product_Idealfor,producr_cover_image, product_isFeatured ,
    ...
    public function get_activeProducts($category_id = '',$maincategory = '',$start = 0,$perPage = 12,$offset='')
    {
    	//$perPage = 4;
		...
		//$this->db->or_where("p1.ManuCtegory_id = ".$maincategory);
        $this->db->where("p.product_status = 'Active'  AND p.product_id = p1.product_id AND p.product_isFeatured!='yes' ");
        $this->db->order_by("p1.product_orderno", "ASC");
	...
	public function get_activeProducts1($maincategory = '',$start = 0,$perPage = 12)
    {   
        $allids=array();
        $catids=$this->get_category($maincategory);
        foreach ($catids as $key => $value) {
            $allids[]=$value['category_id'];
        }
        //print_r($allids);exit();
    	//$perPage = 4;
        ...
        if($category_id != '') {
		 ...
		 $this->db->where_in("p1.category_id",$allids);
        $this->db->where("p.product_status = 'active'  AND p.product_id = p1.product_id");
        $this->db->order_by("p1.product_orderno", "ASC");
        $this->db->limit($perPage,$start);
        $query = $this->db->get();
	    ...
	    public function get_category($maincategory)
    {
    	//$perPage = 4;
        $this->db->select('c.category_name,c.category_slug,c.category_id,c.ManuCtegory_id');
        $this->db->from('rds_categories c');
        $this->db->where("c.ManuCtegory_id = ".$maincategory);
	...
	public function get_activetotalProducts($category_id = '')
    {
		...
		$this->db->where("product_status = 'Active'  AND p.product_id = p1.product_id AND p.product_isFeatured!='yes'");
        $count = $this->db->count_all_results();
        return $count;
    }
}